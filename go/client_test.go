...
func TestSimulateRequests_Success(t *testing.T) {
	client, server := newTestClient(func(w http.ResponseWriter, r *http.Request) {
		if r.Method != http.MethodPost || r.URL.Path != "/simulate" {
			t.Fatalf("request = %s %s, want POST /simulate", r.Method, r.URL.Path)
		}
		var req GroupSignRequest
		if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
			t.Fatalf("decode simulate request: %v", err)
		}
		if len(req.Requests) != 1 || req.Requests[0].AuthAddress != "AUTH" {
			t.Fatalf("simulate requests = %+v", req.Requests)
		}
		json.NewEncoder(w).Encode(GroupSimulateResponse{
			TxIDs:        []string{"SIMTXID1"},
			Transactions: []string{"545801"},
			Mutations:    &MutationReport{DummiesAdded: 1, GroupIDChanged: true},
	...
	result, err := client.SimulateRequests([]SignRequest{{
		TxnBytesHex: "545801",
		AuthAddress: "AUTH",
		TxnSender:   "SENDER",
	}})
	if err != nil {
		t.Fatalf("SimulateRequests() error = %v", err)
	}
	if len(result.TxIDs) != 1 || result.TxIDs[0] != "SIMTXID1" {
		t.Fatalf("txids = %v", result.TxIDs)
	}
	if !result.Failed || !strings.Contains(result.Output, "logic eval error") {
		t.Fatalf("simulate result = %+v", result)
	}
	if result.Mutations == nil || result.Mutations.DummiesAdded != 1 {
		t.Fatalf("mutations = %+v", result.Mutations)
	}
}

func TestSimulatePreparedGroup(t *testing.T) {
	client, server := newTestClient(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/simulate" {
			t.Fatalf("expected /simulate, got %s", r.URL.Path)
		}
		var req GroupSignRequest
		if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
			t.Fatalf("decode simulate request: %v", err)
		}
		if len(req.Requests) != 1 || req.Requests[0].AuthAddress != "AUTH" || req.Requests[0].TxnBytesHex == "" {
			t.Fatalf("simulate requests = %+v", req.Requests)
		}
		json.NewEncoder(w).Encode(GroupSimulateResponse{TxIDs: []string{"SIMTXID1"}})
	})
	defer server.Close()

	txn := types.Transaction{Type: types.PaymentTx}
	result, err := client.SimulatePreparedGroup(context.Background(), NewPreparedGroup(PreparedTransaction{
		Transaction: &txn,
		AuthAddress: "AUTH",
	}))
	if err != nil {
...
func TestSimulateRequests_ResponseError(t *testing.T) {
	client, server := newTestClient(func(w http.ResponseWriter, r *http.Request) {
		json.NewEncoder(w).Encode(GroupSimulateResponse{Error: "simulation unavailable"})
	})
	defer server.Close()

	_, err := client.SimulateRequests([]SignRequest{{TxnBytesHex: "545801", AuthAddress: "AUTH"}})
	if err == nil || !strings.Contains(err.Error(), "simulation unavailable") {
		t.Fatalf("SimulateRequests() error = %v, want simulation unavailable", err)
	}
}
