...
describe("signer API contract fixtures", () => {
  ...
  it("maps /keytypes wire fields to public KeyTypeInfo fields", async () => {
    ...
    assert.equal(genericPolicy.creationParams?.[1].minItems, 1);
    assert.equal(genericPolicy.creationParams?.[1].maxItems, 8);
    assert.equal(genericPolicy.creationParams?.[2].min, 1);
    assert.equal(genericPolicy.creationParams?.[2].max, 999999999);
    assert.equal(genericPolicy.creationParams?.[3].maxLength, 32);
    assert.equal(genericPolicy.creationParams?.[3].inputModes?.[1].name, "sha256");
    assert.equal(genericPolicy.creationParams?.[3].inputModes?.[1].transform, "sha256");
  ...
  it("maps /admin/generate response fields", async () => {
    ...
    assert.equal(generated.keyType, "example.generic-policy.v1");
